...
export default function ImpositionVisualizer({
    ...
    compHeightCm,
    bleedMm,
    printMode = false   // ← when true: raw SVG only, no UI chrome
}) {
                ...
                {Array.from({ length: ups }).map((_, i) => {
                    ...
                    return (
                        <g key={`up-${i}`} className="transition-all duration-300">
                            ...
                            <path
                                d={`M ${finX + finW} ${finY + finH + 5} L ${finX + finW} ${finY + finH + 12} M ${finX + finW + 5} ${finY + finH} L ${finX + finW + 12} ${finY + finH}`}
                                stroke={cellOverflow ? "rgba(239, 68, 68, 0.7)" : "rgba(255, 255, 255, 0.4)"}
                                strokeWidth="0.75"
                            />
    ...
    return (
        <>
            {/* ── Print-mode: raw SVG + minimal stats, no chrome ── */}
            {printMode ? (
                <div className="w-full">
                    ...
                    <div className="flex gap-6 mt-2 text-[10px] text-gray-500 font-mono">
                        ...
                        <span>Sheet: {layout.W?.toFixed(1)} × {layout.H?.toFixed(1)} cm</span>
                        <span>Cut: {layout.cW?.toFixed(1)} × {layout.cH?.toFixed(1)} cm</span>
                        <span>Bleed: {parseFloat(bleedMm || 3).toFixed(1)} mm</span>
                        <span className={layout.fits ? 'text-green-600' : 'text-red-600'}>
                            {layout.utilizationPercent?.toFixed(1)}% utilization
                        </span>
                    </div>
                </div>
            ) : (
