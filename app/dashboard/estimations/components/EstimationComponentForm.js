...
export default function EstimationComponentForm({
    ...
    // Quick-add a finishing by keyword match
    const quickAddFinishing = useCallback((keyword) => {
        ...
        if (alreadyAdded) {
            setShortcutToast({ msg: `${match.name} already added` });
        ...
        setShortcutToast({ msg: `+ ${match.name}` });
        setTimeout(() => setShortcutToast(null), 2000);
    }, [availableFinishings, selectedFinishings, index, onAddFinishing]);

    // Keyboard shortcuts dynamically configured
    useEffect(() => {
                    ...
                    {isServicesComponent ? (
                        ...
                        <>
                                    ...
                                    {(data.name?.includes('Inner') || data.name?.includes('Cover')) || data.name?.includes('Main') && (
                                        <div className="flex items-center gap-3 mt-2">
                                            <button
                                                ...
                                                className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium border transition-all ${isBB
                            ...
                            <div ref={sectionRef}>
                                ...
                                <div className="flex flex-wrap items-center gap-2 border-t border-white/10 pt-4 mb-3">
                                    ...
                                    {sfgLines.map((line) => (
                                        <div key={line.id} className="grid grid-cols-12 gap-2 items-center bg-black/30 border border-amber-500/10 rounded-lg px-3 py-2">
                                            ...
                                            <div className="col-span-2 text-center">
                                                <span className={`text-xs font-mono ${(line.stock_quantity || 0) <= 0 ? 'text-red-400' :
