                        ...
                        <div className="text-[10px] font-bold uppercase tracking-wider mb-2.5 flex items-center justify-between">
                            ...
                            {(machineEmps.length > 0 || machineHelpers.length > 0) && (
                                <span className="text-[9.5px] text-blue-400 bg-blue-500/10 px-2 py-0.5 rounded border border-blue-500/20 font-semibold">
                                    Machine Staff: Operators ({machineEmps.length > 0 ? machineEmps.map(e => e.name).join(', ') : 'any'}){machineHelpers.length > 0 ? ` · Helpers (${machineHelpers.map(e => e.name).join(', ')})` : ''}
                                </span>
                            )}
                        </div>
                        ...
                        <div className="p-6 flex flex-col gap-4 max-h-[80vh] overflow-y-auto">
                            <div className="flex bg-white/5 p-1 rounded-xl border border-white/10 gap-1">
                                <button
                                    ...
                                    className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-all cursor-pointer ${weeklyExportFormat === 'pdf' ? 'bg-emerald-600 text-white shadow-md' : 'text-slate-400 hover:text-white'}`}
                                >
                                    PDF Document
                                </button>
                                <button
                                    onClick={() => setWeeklyExportFormat('csv')}
                                    className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-all cursor-pointer ${weeklyExportFormat === 'csv' ? 'bg-emerald-600 text-white shadow-md' : 'text-slate-400 hover:text-white'}`}
                                >
                                    CSV Spreadsheet
                                </button>
                            </div>
                            ...
                            <div className="flex items-center justify-end gap-2 mt-2 pt-4 border-t border-white/10">
                                ...
                                <button
                                    onClick={() => {
                                        ...
                                        window.open(`/api/job-planning/machine/${selectedMachine.id}/pdf?${params}`, '_blank');
                                    }}
                                    className="px-5 py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold rounded-xl transition-all flex items-center gap-1.5 shadow-lg shadow-emerald-950/20 cursor-pointer"
                                >
                        ...
                        <div className="p-6 flex flex-col gap-4 max-h-[80vh] overflow-y-auto">
                            <div className="flex bg-white/5 p-1 rounded-xl border border-white/10 gap-1">
                                <button
                                    ...
                                    className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-all cursor-pointer ${dailyExportFormat === 'pdf' ? 'bg-emerald-600 text-white shadow-md' : 'text-slate-400 hover:text-white'}`}
                                >
                                    PDF Document
                                </button>
                                <button
                                    onClick={() => setDailyExportFormat('csv')}
                                    className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-all cursor-pointer ${dailyExportFormat === 'csv' ? 'bg-emerald-600 text-white shadow-md' : 'text-slate-400 hover:text-white'}`}
                                >
                                    CSV Spreadsheet
                                </button>
                            </div>
                        ...
                        <div className="p-6 flex flex-col gap-4">
                            <div className="flex bg-white/5 p-1 rounded-xl border border-white/10 gap-1">
                                <button
                                    ...
                                    className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-all cursor-pointer ${exportFormat === 'pdf' ? 'bg-emerald-600 text-white shadow-md' : 'text-slate-400 hover:text-white'}`}
                                >
                                    PDF Document
                                </button>
                                <button
                                    onClick={() => setExportFormat('csv')}
                                    className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-all cursor-pointer ${exportFormat === 'csv' ? 'bg-emerald-600 text-white shadow-md' : 'text-slate-400 hover:text-white'}`}
                                >
                                    CSV Spreadsheet
                                </button>
                            </div>
