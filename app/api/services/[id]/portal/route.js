...
export async function GET(req, { params }) {
    try {
        ...
        const [
            ...
            [[taskStats]],
            [[invoiceStats]],
            [monthlyRevenueRowsRaw],
            [monthlyQuotationsRows],
            [recentQuotationsRaw],
            [recentTasksRaw]
        ] = await Promise.all([
        ...
        let totalRevenue = Number(invoiceStats?.total_collected || 0);
        let totalOutstanding = Number(invoiceStats?.total_outstanding || 0);
        let thisMonthCollected = Number(invoiceStats?.this_month_collected || 0);

        let monthlyRevenueRows = monthlyRevenueRowsRaw;

        // Fallback to Sales Orders for this service if no invoices exist
        if (Number(invoiceStats?.count || 0) === 0 || totalRevenue === 0) {
            ...
            totalRevenue = Number(soRev?.total_collected || 0);
            thisMonthCollected = Number(soRev?.this_month_collected || 0);
        }
