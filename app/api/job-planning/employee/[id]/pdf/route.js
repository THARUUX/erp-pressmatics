...
import EmployeeTasksDocument from './EmployeeTasksDocument';
...
export async function GET(req, { params }) {
    ...
    try {
        ...
        const rawTasks = await JobTaskRepository.getScheduleTasksForEmployee(pool, employee.name, startDateStr, endDateStr);

        let tasks = excludeCompleted ? rawTasks.filter(t => t.status !== 'done') : rawTasks;
        ...
        if (format === 'csv') {
            ...
            return new NextResponse(csvData, {
                ...
                headers: {
                    ...
                    'Content-Disposition': `attachment; filename="employee-schedule-${employee.name.replace(/\s+/g, '-')}.csv"`,
                },
            });
        }
        ...
        tasks.forEach(t => {
            if (!t.scheduled_date) {
                ...
                const dStr = `${y}-${m}-${dd}`;
                if (dailyTasksMap[dStr]) dailyTasksMap[dStr].push(t);
            }
        });
        ...
        const stats = { totalTasks, totalMinutes, completedTasks, pendingTasks, completionRate };
        ...
        const pdfBuffer = await renderToBuffer(
            React.createElement(EmployeeTasksDocument, {
                employee,
                weekRangeStr,
                stats,
                tasksByDay,
        ...
        return new NextResponse(pdfBuffer, {
            ...
            headers: {
                ...
                'Content-Disposition': `attachment; filename="employee-schedule-${employee.name.replace(/\s+/g, '-')}.pdf"`,
            },
        });

    } catch (error) {
        console.error('Employee PDF generation error:', error);
