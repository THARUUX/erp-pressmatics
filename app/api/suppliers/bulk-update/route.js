import { NextResponse } from 'next/server';
import pool from '@/lib/db';

/**
 * PUT /api/suppliers/bulk-update
 * Body: { rows: [{id, name, email, phone, address, contact_name, contact_email, contact_phone, payment_terms, credit_limit, notes}] }
 */
export async function PUT(req) {
    try {
        const { rows } = await req.json();
        if (!Array.isArray(rows) || rows.length === 0) {
            return NextResponse.json({ error: 'No rows provided' }, { status: 400 });
        }

        const updated = [];
        const skipped = [];
        const failed  = [];

        for (const row of rows) {
            const id = parseInt(row.id);
            if (!id || isNaN(id)) {
                skipped.push({ id: row.id, reason: 'Missing or invalid id' });
                continue;
            }
            if (!row.name?.trim()) {
                skipped.push({ id, reason: 'Name is required' });
                continue;
            }
            try {
                const [result] = await pool.execute(
                    `UPDATE suppliers SET
                        code=?, name=?, email=?, phone=?, address=?,
                        contact_name=?, contact_email=?, contact_phone=?,
                        payment_terms=?, credit_limit=?, notes=?
                     WHERE id=?`,
                    [
                        row.code?.trim() || null,
                        row.name.trim(),
                        row.email?.trim() || null,
                        row.phone?.trim() || null,
                        row.address?.trim() || null,
                        row.contact_name?.trim() || null,
                        row.contact_email?.trim() || null,
                        row.contact_phone?.trim() || null,
                        row.payment_terms?.trim() || 'Net 30',
                        parseFloat(row.credit_limit) || 0,
                        row.notes?.trim() || null,
                        id,
                    ]
                );
                if (result.affectedRows > 0) updated.push(id);
                else skipped.push({ id, reason: 'Supplier not found' });
            } catch (err) {
                failed.push({ id, error: err.message });
            }
        }

        return NextResponse.json({ updated: updated.length, skipped, failed });
    } catch (error) {
        console.error('Bulk update suppliers error:', error);
        return NextResponse.json({ error: 'Bulk update failed' }, { status: 500 });
    }
}
