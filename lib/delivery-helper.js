...
export async function syncSalesOrderToDeliveryQueue(salesOrderId, db) {
    try {
        ...
        const [orders] = await db.execute(
            'SELECT id, code, customer_id, customer_name, status, quotation_id FROM sales_orders WHERE id = ?',
            [salesOrderId]
        );
        if (orders.length === 0) {
        ...
        if (order.status !== 'Ready' && order.status !== 'In Production') {
            console.log(`[Delivery Sync] Sales Order #${salesOrderId} is not in 'Ready' or 'In Production' status (current: ${order.status}). Skipping sync.`);
            return;
        }
        ...
        if (qItems.length === 0 && order.code) {
            [qItems] = await db.execute(
                `SELECT id AS quotation_item_id, estimation_name, quantity AS total_quantity
                 FROM quotation_items WHERE code = ?`,
                [order.code]
            );
        }

        if (qItems.length === 0) {
            console.log(`[Delivery Sync] No items found for quotation ID ${order.quotation_id} or order code ${order.code}.`);
            return;
        }
            ...
            if (existing.length === 0) {
                ...
                await db.execute(
                    `INSERT INTO deliveries (
                        ...
                        delivered_quantity, status, delivery_address
                     ) VALUES (?, ?, ?, ?, ?, ?, 0, 'Pending', ?)`,
                    [
                        order.id,
                        order.code,
