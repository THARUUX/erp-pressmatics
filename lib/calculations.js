...
export function calculateOffset({
    ...
    if (Array.isArray(finishings)) {
        computedFinishings = finishings.map(item => {
            ...
            if (costUnit === 'Page') {
                ...
                const cutW = (parseFloat(compWidthCm) || parseFloat(cutWidthCm) || 21.0);
                const cutH = (parseFloat(compHeightCm) || parseFloat(cutHeightCm) || 29.7);
    ...
    const isFinishingComp = (compName || '').toLowerCase().includes('finishing');
