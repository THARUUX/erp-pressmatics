...
export function calculateDigital({
    ...
    // Dimensions & Pricing
    const width = parseFloat(compWidthCm) || parseFloat(cutWidthCm) || parseFloat(paperWidthCm) || 0;
    const height = parseFloat(compHeightCm) || parseFloat(cutHeightCm) || parseFloat(paperHeightCm) || 0;
    ...
    if (Array.isArray(finishings)) {
        computedFinishings = finishings.map(item => {
            ...
            if (costUnit === 'Page') {
                ...
                const cutW = parseFloat(compWidthCm) || parseFloat(cutWidthCm) || 0;
                const cutH = parseFloat(compHeightCm) || parseFloat(cutHeightCm) || 0;
                const widthInches = ((cutW / 2.54) + 0.5);
                const heightInches = ((cutH / 2.54) + 0.5);
                const sqInQty = widthInches * heightInches * (qty + wastageCutSheets);
                unitQty = sqInQty;
