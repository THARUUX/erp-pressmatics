 ...
 *
 * @param {number} speed        - Machine speed (numeric value, > 0)
 * @param {string} unit         - Speed unit string from DB / form
 * @param {object} counts       - { totalCutSheets, sides, impressions, qty }
 * @returns {number}            - Run time in hours
 *
 * Supported units:
 *   Prints/Hr      → (totalCutSheets × sides) / speed
 *   Sheets/Hr      → totalCutSheets / speed
 *   Impressions/Hr → impressions / speed
 *   Units/Hr       → qty / speed
 *   Copies/Hr      → qty / speed
 *   Pcs/Hr         → qty / speed
 *   (fallback)     → qty / speed
 */
export function calcTimeHours(speed, unit, { totalCutSheets = 0, sides = 1, impressions = 0, qty = 0 } = {}) {
    if (!speed || speed <= 0) return 0;
    const u = (unit || 'Sheets/Hr').toLowerCase().trim();
    if (u === 'prints/hr') {
        return (totalCutSheets * sides) / speed;
    } else if (u === 'sheets/hr') {
        return totalCutSheets / speed;
    } else if (u === 'impressions/hr') {
        return impressions / speed;
    } else {
        // Units/Hr, Copies/Hr, Pcs/Hr, and any unknown unit → qty
        return qty / speed;
    }
}
...
export function calculateDigital({
    ...
    // For digital: "cut sheets" = printedSheets (already accounts for ups),
    // impressions = printedSheets × sides.
    let printingTime = 0;
    if (machineSpeed && parseFloat(machineSpeed) > 0) {
        printingTime = calcTimeHours(parseFloat(machineSpeed), machineSpeedUnit, {
            totalCutSheets: printedSheets,
            sides: sidesVal,
            impressions: printedSheets * sidesVal,
            qty
        });
    }
