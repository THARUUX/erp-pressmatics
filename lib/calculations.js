...
export function calculateDigital({
    ...
    const TotalImpressions = Math.ceil(qty / upsVal);

    // Core costs
    const paperCost = TotalImpressions * (parseFloat(paperCostPerSheet) || 0);
    ...
    const printingCost = TotalImpressions * width * height * priceSqCm * sidesVal;

    // PRINTING TIME (Digital)
    // For digital: "cut sheets" = TotalImpressions (already accounts for ups),
    // impressions = TotalImpressions × sides.
    let printingTime = 0;
    if (machineSpeed && parseFloat(machineSpeed) > 0) {
        printingTime = calcTimeHours(parseFloat(machineSpeed), machineSpeedUnit, {
