...
export async function generateJobTasks(id, customConn = null) {
    ...
    const [finishingsList] = await db.execute('SELECT id, name FROM finishings');
        ...
        for (const taskNameBase of Object.keys(grouped)) {
            ...
            if (finConfig.is_bb_separated) {
                ...
                for (const g of group.items) {
                    ...
                    const mObj = f.machine_id ? machineMap.get(f.machine_id) : null;
                    const speedUnit = f.speed_unit || f.machine_speed_unit || f.cost_unit || mObj?.speed_unit || '';
    ...
    if (qcConfig && qcConfig.is_enabled) {
        taskList.push({
            ...
            machine_name: qcConfig.machine_id ? machineMap.get(qcConfig.machine_id) : (qcConfig.finishing_id ? finishingMap.get(qcConfig.finishing_id) : null),
            finishing_id: qcConfig.finishing_id || null,
    ...
    if (packingConfig && packingConfig.is_enabled) {
        taskList.push({
            ...
            machine_name: packingConfig.machine_id ? machineMap.get(packingConfig.machine_id) : (packingConfig.finishing_id ? finishingMap.get(packingConfig.finishing_id) : null),
            finishing_id: packingConfig.finishing_id || null,
    ...
    if (deliveryConfig && deliveryConfig.is_enabled) {
        taskList.push({
            ...
            machine_name: deliveryConfig.machine_id ? machineMap.get(deliveryConfig.machine_id) : (deliveryConfig.finishing_id ? finishingMap.get(deliveryConfig.finishing_id) : null),
            finishing_id: deliveryConfig.finishing_id || null,
    ...
    for (const cc of customConfigs) {
        if (cc.is_enabled) {
            taskList.push({
                ...
                machine_name: cc.machine_id ? machineMap.get(cc.machine_id) : (cc.finishing_id ? finishingMap.get(cc.finishing_id) : null),
                finishing_id: cc.finishing_id || null,
    ...
    for (const t of taskList) {
        await db.execute(
            'INSERT INTO job_tasks (sales_order_id, name, description, machine_id, machine_name, assigned_to, display_order, estimated_minutes, quantity, sheet_count, impression_count, service_id, finishing_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
            [parseInt(id), t.name, t.description || null, t.machine_id || null, t.machine_name || null, t.assigned_to || null, t.display_order, t.estimated_minutes || null, t.quantity || null, t.sheet_count || null, t.impression_count || null, t.service_id || null, t.finishing_id || null]
        );
    }
